...
# ---------------------------------------------------------------------------
# Reference state helpers  (JSON + Parquet, no pickle)
# ---------------------------------------------------------------------------

def load_reference_state(ref_dir: Path) -> tuple[dict, dict]:
    """
    Load master entity DataFrames and ID counters saved by generate_initial_load.py.
    Returns (dataframes_dict, counters_dict).
    """
...
def save_reference_state(ref_dir: Path, dataframes: dict, counters: dict) -> None:
    """Persist updated master entity DataFrames and counters after an incremental run."""
    mapping = {
...
def apply_customer_scd2_changes(
    ...
    df      = df.copy().reset_index(drop=True)  # guarantee positional RangeIndex
    n       = len(df)
    summary = {}

    # City changes — per-row loop required: pool depends on each row's country
    # and must exclude the customer's current city.
    # FIX: use iloc (positional) instead of at/loc (label-based) throughout.
    ...
    # Email + phone changes — vectorized.
    # FIX: use iloc for assignment (positional), not loc (label-based).
...
def apply_employee_offline_scd2(
    ...
    # FIX: use iloc (positional) instead of loc (label-based).
