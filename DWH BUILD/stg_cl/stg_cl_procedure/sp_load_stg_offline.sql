...
CREATE OR REPLACE PROCEDURE stg_cln.sp_load_stg_offline()
LANGUAGE plpgsql AS $$
DECLARE
    v_log_id        BIGINT;
    v_inserted      INT := 0;
    ...
    v_err_msg       TEXT;
BEGIN
    INSERT INTO bl_cn.etl_log (procedure_name, layer, status)
    VALUES ('stg_cln.sp_load_stg_offline', 'STG_CLN', 'STARTED')
    ...
    WHERE
        ...
        AND COALESCE(
                NULLIF(REGEXP_REPLACE(discount_applied,'[^0-9.]','','g'),'')::NUMERIC,
                0
            ) BETWEEN 0 AND 100;

    GET DIAGNOSTICS v_inserted = ROW_COUNT;

    INSERT INTO stg_cln.reject_sales (
        source_system,
        source_table,
        reject_reason,
        raw_data
    ...
    SELECT
        ...
        CONCAT_WS(' | ',
            CASE WHEN customer_id IS NULL
                      OR TRIM(customer_id) = ''
                 THEN 'NULL/EMPTY customer_id'                          END,
            CASE WHEN product_id IS NULL
                      OR TRIM(product_id) = ''
                 THEN 'NULL/EMPTY product_id'                           END,
            CASE WHEN transaction_id IS NULL
                      OR TRIM(transaction_id) = ''
                 THEN 'NULL/EMPTY transaction_id'                       END,
            CASE WHEN transaction_date IS NULL
                      OR transaction_date NOT LIKE '____-__-__'
                 THEN 'INVALID transaction_date: ' || COALESCE(transaction_date,'NULL') END,
            CASE WHEN NULLIF(REGEXP_REPLACE(
                        quantity_sold,'[^0-9]','','g'),'')::INTEGER <= 0
                 THEN 'quantity_sold must be > 0'                       END,
            CASE WHEN NULLIF(REGEXP_REPLACE(
                        sales_amount,'[^0-9.]','','g'),'')::NUMERIC <= 0
                 THEN 'sales_amount must be > 0'                        END,
            CASE WHEN COALESCE(NULLIF(REGEXP_REPLACE(
                        discount_applied,'[^0-9.]','','g'),'')::NUMERIC,0)
    ...
    WHERE
        ...
        OR COALESCE(NULLIF(REGEXP_REPLACE(
                discount_applied,'[^0-9.]','','g'),'')::NUMERIC,0)
           NOT BETWEEN 0 AND 100;
    ...
    CALL bl_cn.log_success(v_log_id, v_inserted);
        ...
        RAISE NOTICE '[sp_load_stg_offline] Total: % | Valid: % | Rejected: %',
