...
BEGIN
    ...
    SELECT
        ...
        t.transaction_sale_amount - (t.transaction_quantity_sold * dp_p.product_unit_cost),
        CASE
            WHEN t.transaction_sale_amount <> 0
            THEN (t.transaction_sale_amount - (t.transaction_quantity_sold * dp_p.product_unit_cost)) / t.transaction_sale_amount
            ELSE NULL 
        END,
        NOW(), NOW()
    FROM bl_3nf.ce_transactions t
    LEFT JOIN bl_dm.dm_products dp ON dp.product_src_id = t.product_id
          AND dp.source_system  = 'BL_3NF'
          AND dp.source_entity  = 'CE_PRODUCTS'
    LEFT JOIN bl_dm.dm_products dp_p ON dp_p.product_src_id = t.product_id
          AND dp_p.source_system  = 'BL_3NF'
          AND dp_p.source_entity  = 'CE_PRODUCTS'
    ...
    LEFT JOIN bl_dm.dm_employees_scd de ON de.employee_src_id = t.employee_id
          AND de.source_system   = 'BL_3NF'
          AND de.source_entity   = 'CE_EMPLOYEES_SCD'
    LEFT JOIN bl_dm.dm_store_branches dsb ON dsb.store_branch_src_id = t.store_branch_id
          AND dsb.source_system       = 'BL_3NF'
          AND dsb.source_entity       = 'CE_STORE_BRANCHES'
    LEFT JOIN bl_dm.dm_shippings dsh ON dsh.shipping_src_id = t.shipping_id
    ...
    LEFT JOIN bl_dm.dim_dates dd_deliv
           ON dd_deliv.full_date = t.transaction_delivery_dt
    WHERE NOT EXISTS (
...
EXCEPTION WHEN OTHERS THEN
    ...
    WHERE log_id = v_log_id;
    RAISE EXCEPTION '[load_fct_transactions_dd] FAILED: %', v_err_msg;
END;
$$;
