
--  Employee entity - SCD Type 2

CREATE TABLE IF NOT EXISTS bl_3nf.ce_employees_scd (
    employee_id           BIGINT         PRIMARY KEY
                          DEFAULT nextval('bl_3nf.seq_ce_employees_scd'),
    employee_src_id       VARCHAR(15)    NOT NULL,
    store_branch_id       BIGINT         NOT NULL
                          REFERENCES bl_3nf.ce_store_branches(store_branch_id),
    employee_firstname    VARCHAR(50)    NOT NULL,
    employee_lastname     VARCHAR(50)    NOT NULL,
    employee_title        VARCHAR(50),
    employee_email        VARCHAR(100),
    employee_phone_number VARCHAR(20),
    employee_salary       NUMERIC(12,2),        
    start_dt              DATE           NOT NULL DEFAULT CURRENT_DATE,
    end_dt                DATE           NOT NULL DEFAULT '9999-12-31',
    is_active             BOOLEAN        NOT NULL DEFAULT TRUE,
    row_hash              VARCHAR(32)    NOT NULL DEFAULT 'n. a.',
    ta_insert_dt          TIMESTAMP      NOT NULL DEFAULT NOW(),
    source_system         VARCHAR(20)    NOT NULL,
    source_entity         VARCHAR(50)    NOT NULL,

    CONSTRAINT uq_ce_employees_version UNIQUE (employee_src_id, source_system, source_entity, start_dt)
);

