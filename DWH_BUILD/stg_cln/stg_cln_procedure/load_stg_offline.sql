...
BEGIN
    ...
    FROM (
        ...
        ORDER BY TRIM(transaction_id), ctid   
    ) deduped;
    ...
    SELECT
        ...
        CONCAT_WS(' | ',
            ...
            CASE WHEN COALESCE(NULLIF(REGEXP_REPLACE(quantity_sold,    '[^0-9]',  '', 'g'), '')::INTEGER, -1) <= 0
                 THEN 'quantity_sold must be > 0'                                                 END,
            CASE WHEN COALESCE(NULLIF(REGEXP_REPLACE(sales_amount,     '[^0-9.]', '', 'g'), '')::NUMERIC,  -1) <= 0
